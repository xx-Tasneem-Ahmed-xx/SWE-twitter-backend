// src/tests/chat.test.ts (top of file)
jest.mock("@/app", () => {
  return {
    // we only need what messagesController uses
    socketService: {
      sendMessageToChat: jest.fn(),
      // add other methods if used
    },
    // if other code expects `app` or `io` shape, add minimal stubs
  };
});
...
describe("chat service", () => {
    ...
    describe("send a new message", () => {
        ...
        it("send a message without a chat ID should create a new chat", async () => {
            ...
            await chatService.addMessageToChat(req, res, next);
    ...
    describe("get unseen chats count", () => {
        it("should retrieve unseen chats count successfully", async () => {
            const chat = await chatService.CreateChatFun(true, ["123", "456"]) as any;
            const res = mockRes();
            const req: any = { user: { id: "123" } };
            const next = jest.fn();
            await chatService.getUnseenChatsCount(req, res, next);
            const countObj = res.json.mock.calls[0][0];
            expect(res.status).toHaveBeenCalledWith(200);
            expect(countObj).toHaveProperty("unseenChatsCount", 0);
        });
        it("should return 400 for missing user ID when retrieving unseen chats count", async () => {
            const res = mockRes();
